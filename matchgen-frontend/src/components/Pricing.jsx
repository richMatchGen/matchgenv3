...
export default function Pricing() {
  return (
    <Container id="pricing" sx={{ pt: { xs: 4, sm: 12 }, pb: { xs: 8, sm: 16 }, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: { xs: 3, sm: 6 } }}>
      ...
      <Grid container spacing={3} sx={{ alignItems: 'center', justifyContent: 'center', width: '100%' }}>
        {tiers.map((tier) => (
          <Grid item xs={12} sm={tier.title === 'Enterprise' ? 12 : 6} md={4} key={tier.title}>
            <Card sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 4, ...(tier.title === 'Professional' && { background: 'radial-gradient(circle at 50% 0%, hsl(220, 20%, 35%), hsl(220, 30%, 6%))' }) }}>
              <CardContent>
                <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2, ...(tier.title === 'Professional' && { color: 'grey.100' }) }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'baseline', ...(tier.title === 'Professional' && { color: 'grey.50' }) }}>
                ...
                {tier.description.map((line) => (
                  <Box key={line} sx={{ py: 1, display: 'flex', gap: 1.5, alignItems: 'center' }}>
                    ...
                    <Typography variant="subtitle2" component="span" sx={tier.title === 'Professional' ? { color: 'grey.50' } : {}}>{line}</Typography>
                  </Box>
                ))}
              </CardContent>
              <CardActions>
